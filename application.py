...
questions = [{
  ...
  'name': 'network',
  'choices': ['Busiest Day', 'Weekdays', 'Weekend', 'All Days']
}, {
  'type': 'list',
...
routes, stops, stop_times, trips, shapes = gtfs.import_gtfs('./feeds/' + GTFS, busiest_date = True)
print('GTFS imported in ' + str(round((time.time() - start) / 60, 3)) + ' minutes')

#Filtering the GTFS to the appropriate corridor
if (network == 'Enter Route List') & (check_routes(route_list)): 
  print('Filtering gtfs for routes: ' + str(route_list))
  routes = routes[routes.route_short_name.isin(route_list)]
...
trips = trips[trips.route_id.isin(routes.route_id.unique()) & (trips.trip_id.isin(trips_in_period))]
...
if answers['direction'] != 'Both Ways': 
  combined = seg_freq[seg_freq.dir_id == answers['direction']].groupby(['segment_id', 'window', 's_st_id', 's_st_name', 'e_st_name']).agg({'frequency': 'sum', 'ntrips': 'sum', 'geometry': 'first'}).reset_index()
...
combined['headway'] = round((1 / (combined.frequency / hours)) * 60, 1)
...
ax = combined.plot(figsize=(20, 20), column = 'frequency', cmap = 'inferno', 
